...
import { ICertAddress, defaultCertAddress } from '@/types/certAddress';
...
import { getCertAddress } from './action';

const items: MenuItem[] = [{ label: 'Database' }, { label: 'ข้อมูลที่อยู่ Certificate' }];
...
function LabsPage() {
  ...
  const [data, setData] = useState<ICertAddress[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [labs, setLabs] = useState<ICertAddress>(defaultCertAddress);
  ...
  const get_instrument_data = async () => {
    try {
      setLoading(true);
      const items = await getCertAddress();
      setData(items)
      setLoading(false);
    } catch (err) {
  ...
  const action_display = (item: ICertAddress) => {
    return (
  ...
  const edit_handle = (item: ICertAddress) => { }
        ...
        <div className='col-12 md:col-12 mt-2'>
          <div className="card ">
            ...
            <DataTable
              ...
              value={data}
              filters={filters}
              globalFilterFields={['cert_id', 'cert_name', 'cert_address']}
              stripedRows
              header={header_display}
              paginator rows={15}
              tableStyle={{ width: '100%' }}>
              <Column field='cert_id' header="Id" ></Column>
              <Column field='cert_name' header="Cert. Name" style={{ width: '30%' }} ></Column>
              <Column field='cert_address' header="Cert. Address" style={{ width: '53%' }} ></Column>
