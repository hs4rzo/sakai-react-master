...
import { ITaxRate, defaultCustomer, defaultEthanol, defaultTaxRate } from '@/types/customer';
...
const items: MenuItem[] = [{ label: 'Database' }, { label: 'อัตราภาษี' }];
...
function LabsPage() {
  ...
  const [data, setData] = useState<ITaxRate[]>([]);
  const [loading, setLoading] = useState<boolean>(false);
  const [labs, setLabs] = useState<ITaxRate>(defaultTaxRate);
...
const deleteItem = async (tax_id) => {
 
};

const edit_handle = (item: ITaxRate) => { }
...
const get_instrument_data = async () => {
  try {
    setLoading(true);
    const response = await fetch('/tax.json');
...
const action_display = (item: ITaxRate) => {
  return (
    <div style={{ width: '100%' }} className='flex justify-content-between'>
      ...
      <Button
        ...
        onClick={async () => {
          ...
          if (confirmDelete) {
            await deleteItem(item.tax_id); // ตรวจสอบว่ามี item.atg_id
          }
        }}
      />
    </div>
      ...
      <div className="grid">
        ...
        <div className='col-12 md:col-12 mt-2'>
          ...
          <DataTable
            ...
            scrollable
            filters={filters}
            globalFilterFields={['tax_name', 'tax_rate', 'description']} // กำหนดฟิลด์สำหรับการค้นหา
            stripedRows
            header="อัตราภาษี"
            paginator
            rows={15}
        >
            ...
            <Column field="tax_id" header="รหัสภาษี" />
            <Column field="tax_name" header="ชื่อภาษี" />
            <Column field="tax_rate" header="อัตราภาษี (%)" />
            <Column field="description" header="รายละเอียด" />
            <Column field="effective_date" header="วันที่มีผลบังคับใช้" />
            <Column field="expiration_date" header="วันหมดอายุ" body={rowData => rowData.expiration_date || 'N/A'} />
        </DataTable>
